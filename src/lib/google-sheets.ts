...
function rowToVoucher(row: string[]): Voucher {
  return {
    ...
    voucherType: (row[15] as import("@/types").VoucherType) || "otro",
  };
}

export async function getVouchers(accessToken: string, contactId?: string): Promise<Voucher[]> {
  const rows = await readSheet(accessToken, "Vouchers!A2:P");
...
export async function setVoucherCalendarEvent(
  ...
  await updateRow(accessToken, "Vouchers", rowIndex + 2, [
    ...
    v.checkIn, v.checkOut, calendarEventId, v.notes || "", v.voucherType || "otro",
  ]);
}
...
export async function updateVoucher(
  accessToken: string,
  id: string,
  patch: Partial<Pick<Voucher, "description" | "notes" | "date" | "checkIn" | "checkOut" | "voucherType">>
): Promise<void> {
  const rows = await readSheet(accessToken, "Vouchers!A2:P");
  const rowIndex = rows.findIndex((r) => r[0] === id);
  if (rowIndex === -1) throw new Error("Voucher not found");
  ...
  await updateRow(accessToken, "Vouchers", rowIndex + 2, [
    ...
    v.calendarEventId,
    patch.notes ?? v.notes,
    patch.voucherType ?? v.voucherType ?? "otro",
  ]);
}
...
function rowToMktMetrics(row: string[]): MktMetrics {
  return {
    ...
    rrssNewFollowers: Number(row[4]) || 0, rrssReach: Number(row[5]) || 0,
    ...
    rrssViews: Number(row[16]) || 0,
    wspCatalogViews: Number(row[17]) || 0,
    wspStatusViews: Number(row[18]) || 0,
  };
}

export async function getMktMetrics(accessToken: string): Promise<MktMetrics[]> {
  try {
    const rows = await readSheet(accessToken, "MktMetrics!A2:S");
...
export async function upsertMktMetrics(
  ...
  const toRow = (id: string, createdAt: string) => [
    ...
    String(data.rrssNewFollowers), String(data.rrssReach), String(data.rrssEngagements),
    ...
    data.periodFrom || "", data.periodTo || "", String(data.rrssViews),
    String(data.wspCatalogViews || 0), String(data.wspStatusViews || 0),
  ];
  if (rowIndex === -1) {
    const id = generateId();
